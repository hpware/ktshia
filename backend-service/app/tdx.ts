...
async function getNewToken(clientId: string, clientSecret: string): Promise<string> {
...
export async function getAlerts() {
    const token = await getToken(tdxClientId, tdxClientSecret);
    const req = await fetch("https://tdx.transportdata.tw/webapi/alertInfo/count/bus");
